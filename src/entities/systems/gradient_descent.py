...
class GradientDescentSurfaceSystem:
    ...
    def update(registry: Registry, time: float, delta_time: float):
        for g_entity, (g_transform, g_visuals, g_surface, g_hierarchy) in registry.view(Transform, Visuals, GradientDescentSurface, Hierarchy):
            ...
            if g_surface.dirty:
                GradientDescentSurfaceSystem._generate_mesh(g_visuals, g_surface)
                g_surface.dirty = False

                for o_entity in g_hierarchy.children:
    ...
    @staticmethod
    def _generate_mesh(visuals: Visuals, surface: GradientDescentSurface):
